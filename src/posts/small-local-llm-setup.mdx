...
This tutorial is geared towards anyone with a modern / moderately powered computer, preferably with an Nvidia GPU. My goal here is to detail a minimum working setup that can be extended to meet your needs. We will just use one model in this tutorial, [Qwen 3.5 9B](https://huggingface.co/unsloth/Qwen3.5-9B-GGUF) at 4-bit quantization.

## My System
I ran all of this on an Ubuntu 24.04 server.
- Intel i7-12700K
- 48GB DDR4-3200 RAM
- 2 x 1TB NVMe SSDs
...
Your configuration will vary; that is fine. I will assume that you have an Nvidia GPU, preferably with 8GB VRAM. If the models don't fit on your GPU, they can overflow to system RAM, but performance will take a hit.

Instructions for running on the CPU only are at the very end of this post.
...
**Note:** For a smaller compute setup, there are [smaller versions of Gemma 3](https://huggingface.co/collections/unsloth/gemma-3) available. Look for the `UD-Q4_K_XL` variant in the 270M, 1B, 4B, or 12B sizes. Make sure to download the matching `mmproj-BF16.gguf` file.

Once you finish downloading, your root project directory should look like:
```
└── models
    ├── gemma-3
...
- We are using [llama.cpp](https://github.com/ggml-org/llama.cpp) with [Llama Router](https://huggingface.co/blog/ggml-org/model-management-in-llamacpp) to run the models. I have tried Ollama and llama.cpp with llama-swap, and nothing has worked for me as well as what I have here.
